...
/// `pw`-only rendering of [ResolvedCv] in the `structured_serif` style —
...
/// features vs. `ats_minimal`: centered, rule-less section headings; a
...
List<pw.Widget> buildStructuredSerifPdfContent(
  ResolvedCv cv,
...
pw.Widget _header(
  ...
  return pw.Column(
    ...
    // `ats_minimal_pdf_renderer.dart`'s `_header` doc comment for why a
        ...
        pw.Wrap(
          ...
          children: [
            for (var i = 0; i < contactParts.length; i++) ...[
              ...
              // in `ats_minimal_pdf_renderer.dart` for why one shared
...
/// both margins, not ragged-right like `ats_minimal`'s.
...
/// `ats_minimal`'s single-row project link does.
