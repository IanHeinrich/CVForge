...
/// between the findings list, the Machine Ingestion panel (raw extracted
/// text), and the X-Ray panel (currently just its proof-of-concept
/// checkpoint — one box on one word, see `docs/
/// ats-xray-overlay-handover.md` §6). The eventual "done" shape is a
/// two-pane desktop/tabbed-compact split per §4, not three tabs — this is
/// the incremental-build-order shape, not the final one.
                ...
                children: [
                  result.findings.isEmpty
                      ? const AppEmptyState(
                          ...
                          message:
                              'Nothing in this PDF matched a known ATS '
                              'parsing problem.',
                        )
                      : ListView.separated(
